...
class LETTER(T5ForConditionalGeneration):
    ...
    def __init__(self, config: T5Config, dataset, model_type, task):

        super().__init__(config)

        self.temperature = 1.0
        self.test_sparse_index = json.load(open(f"./rag_need/{model_type}/{dataset}/{task}/test/rag_user_index.json"))
        self.test_sas_index = json.load(open(f"./rag_need/{model_type}/{dataset}/{task}/test/item_retrival.json"))
        self.test_sparse_index = np.array(self.test_sparse_index)
        self.test_sas_index = np.array(self.test_sas_index)
        self.user_mean_emb = np.load(f"./rag_need/{model_type}/{dataset}/{task}/test/user_emb_mean.npy")
        self.text_user_mean_emb = np.load(f"./rag_need/{model_type}/{dataset}/{task}/test/text_user_emb_mean.npy")
        self.test_rerank = json.load(open(f"./rag_need/{model_type}/{dataset}/{task}/test/reranked_user_index.json"))
    ...
    def forward(
        ...
        **kwargs,
        ...
        if user_rag_emb is not None and text_encoder_outputs is None:
            fusion_hidden_states = torch.cat((hidden_states[:,:4,:], user_rag_emb, hidden_states[:,4:,:]), dim=1)
...
class final_model(nn.Module):
    ...
    def __init__(self, T5config, args):
        ...
        task = args.task
        self.id_model = LETTER(T5config, dataset, args.model_type,task)
        self.text_model = LETTER(T5config, dataset, args.model_type,task)
        ...
        self.train_user_mean_emb = np.load(f"./rag_need/{args.model_type}/{dataset}/{task}/train/user_emb_mean.npy")
        self.val_user_mean_emb = np.load(f"./rag_need/{args.model_type}/{dataset}/{task}/val/user_emb_mean.npy")
        self.train_text_user_mean_emb = np.load(f"./rag_need/{args.model_type}/{dataset}/{task}/train/text_user_emb_mean.npy")
        self.val_text_user_mean_emb = np.load(f"./rag_need/{args.model_type}/{dataset}/{task}/val/text_user_emb_mean.npy")

        self.train_sparse_index = json.load(open(f"./rag_need/{args.model_type}/{dataset}/{task}/train/rag_user_index.json"))
        self.train_sparse_index = np.array(self.train_sparse_index)
        self.train_sas_index = json.load(open(f"./rag_need/{args.model_type}/{dataset}/{task}/train/item_retrival.json"))
        self.train_sas_index = np.array(self.train_sas_index)
        self.val_sparse_index = json.load(open(f"./rag_need/{args.model_type}/{dataset}/{task}/val/rag_user_index.json"))
        self.val_sparse_index = np.array(self.val_sparse_index)
        self.val_sas_index = json.load(open(f"./rag_need/{args.model_type}/{dataset}/{task}/val/item_retrival.json"))
        self.val_sas_index = np.array(self.val_sas_index)
        self.train_rerank = json.load(open(f"./rag_need/{args.model_type}/{dataset}/{task}/train/reranked_user_index.json"))
        self.train_rerank = np.array(self.train_rerank)
        self.val_rerank = json.load(open(f"./rag_need/{args.model_type}/{dataset}/{task}/val/reranked_user_index.json"))
    ...
    def Con_loss(self, rep1: torch.Tensor, rep2: torch.Tensor):
        batch_size, seq_len, d = rep1.size()
        logits = torch.matmul(rep1, rep2.T) / 0.9 
        labels = torch.arange(rep1.size(0), device=rep1.device)

        
        contrastive_mask = self.create_contrastive_mask(seq_len, batch_size, device=rep1.device)

        
        masked_logits = logits.masked_fill(contrastive_mask, -1e9)
    ...
    def forward(self, batch, epoch):
        ...
        index = batch['index']
        sim_sparse_user_index = self.train_sparse_index[index][:,:50][:, ::-1]
        sim_sas_user_index = self.train_sas_index[index][:,:50][:, ::-1]
        sim_reranked_user_index = self.train_rerank[index][:,:50]
    ...
    def evaluate(self, batch, index):
        ...
        labels = batch['lm_inputs']['labels'].to(device)
        index = batch['index']
        sim_sparse_user_index = self.val_sparse_index[index][:,:50][:, ::-1]
        sim_sas_user_index = self.val_sas_index[index][:,:50][:, ::-1]
        sim_reranked_user_index = self.val_rerank[index][:,:50]
        input_item_idx = batch['text_input_ids'].to(device)
        input_item_mask = batch['text_attention_mask'].to(device)
    ...
    def predict(self, batch, prefix_allowed_tokens):
        ...
        with torch.no_grad():
            ...
            sim_sparse_user_index = self.id_model.test_sparse_index[index][:,:50][:, ::-1]
            sim_sas_user_index = self.id_model.test_sas_index[index][:,:50][:, ::-1]
            sim_reranked_user_index = self.id_model.test_rerank[index][:,:50]

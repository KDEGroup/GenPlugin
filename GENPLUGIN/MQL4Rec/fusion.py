...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='tiger', help='tiger / letter /mutil')
    parser.add_argument('--test_task', type=str, default='seqrec', help='seqrec / seqimage')
...
def fusion_embedding(args):
    ...
    for mode in modes:
        ...
        folder_path = f'../MQL4Rec-RAR/{model_type}/{dataset}/{test_task}/{mode}/'
        ...
        np.save(f'../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/{mode}/user_emb_mean.npy', user_emb_mean)
        np.save(f'../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/{mode}/text_user_emb_mean.npy', text)
        np.save(f'../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/{mode}/decoder_item_emb_mean.npy', decoder)
...
def rerank(args):
    ...
    top_k = 50
    for mode in modes:
        sparse_index =  json.load(open(f"../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/{mode}/rag_user_index.json"))
        cf_index = json.load(open(f"../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/{mode}/item_retrival.json"))
        if mode == 'train':
            user_emb_mean_aug = np.load(f'../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/aug_train/user_emb_mean.npy')
        user_emb_mean = np.load(f'../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/{mode}/user_emb_mean.npy')
        reranked_index = []
        dict = {}
        ...
        save_path = f"../MQL4Rec-RAR/rag_need/{model_type}/{dataset}/{test_task}/{mode}/reranked_user_index.json"

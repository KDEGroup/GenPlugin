...
def test(args):
    ...
    if args.test_mode == 'test':
        model_state_dict = torch.load(os.path.join(args.output_dir, args.dataset+'/pytorch_model.bin'), map_location=device)
    ...
    if args.test_mode == 'test':
        ...
        import shutil
        output_dir = f"../MQL4Rec-RAR/mutil/{args.dataset}/{args.test_task}/{args.data_mode}"
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(tqdm(test_loader)):
                ...
                output = model.predict(batch, prefix_allowed_tokens=prefix_allowed_tokens,test_task=args.test_task, test_mode=args.test_mode, data_mode=args.data_mode, index=step)
    else:
        raise ValueError("test_mode should be test or rag")

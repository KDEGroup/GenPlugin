...
class final_model(nn.Module):
    ...
    def __init__(self, T5config, args):
        ...
        image_emb = np.load(f'../data/{self.dataset_name}/{self.dataset_name}.emb-ViT-L-14.npy')
    ...
    def predict(self, batch, prefix_allowed_tokens, test_task='seqrec', test_mode='test', data_mode='test', index=None):
        ...
        with torch.no_grad():
            ...
            if test_mode == 'test':
                ...
                np.save(f'../MQL4Rec-RAR/mutil/{self.dataset_name}/{test_task}/{data_mode}/{index}.mean.npy', encoder_output_mean.cpu().detach().numpy())
                np.save(f'../MQL4Rec-RAR/mutil/{self.dataset_name}/{test_task}/{data_mode}/{index}.decoder.mean.npy', decoder_output_mean.cpu().detach().numpy())
                np.save(f'../MQL4Rec-RAR/mutil/{self.dataset_name}/{test_task}/{data_mode}/{index}.text.mean.npy', text_model_output_mean.cpu().detach().numpy())
        return output

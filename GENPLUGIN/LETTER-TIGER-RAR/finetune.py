...
def train(gpu, args):
    ...
    if args.train_mode == 'rag':
        if args.model_type == 'tiger':
            model_state_dict = torch.load(f'../LETTER-TIGER/ckpt/{args.dataset}/tiger-rag/pytorch_model.bin')
        else:
            model_state_dict = torch.load(f'../LETTER-TIGER/ckpt/{args.dataset}/letter-rag/pytorch_model.bin')

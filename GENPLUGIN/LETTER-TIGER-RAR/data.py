...
class SeqRecDataset(BaseDataset):
    ...
    def _get_sim_item(self, item_ids, item_history, topk=4):
        assert len(item_ids) <= 20 and len(item_history) <= 20
        temp_item_ids = []
        temp_history = []
        for i in range(len(item_ids)):
            sim_items = self.item_sims[str(item_ids[i])]
            sim_history = self.sim_remap_inters[str(item_ids[i])]
            sim_items.reverse()
            sim_history.reverse()
            for j in range(topk):
                    temp_item_ids.append(sim_items[j])
                    temp_history.append(sim_history[j])
        assert len(temp_item_ids) == len(item_ids) * topk and len(temp_item_ids) <= 80
        assert len(temp_history) == len(item_ids) * topk and len(temp_history) <= 80
        return temp_item_ids, temp_history

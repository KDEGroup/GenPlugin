...
class final_model(nn.Module):
    ...
    def __init__(self, T5config, args):
        ...
        text_emb = np.load(f'../data/{self.dataset_name}/{self.dataset_name}.emb-two-llama2-td.npy')
    ...
    def item_con_loss(self, normal, tail):
        batch_size, seq_length, _ = tail.size()
    ...
    def forward(self, batch, epoch):
        ...
        text_inputs = {
            'inputs_embeds': self.projection_head(self.text_emb[batch['text_input_ids'].to(device)]),
        ...
        if random.random() < self.get_prob(epoch, start_epoch=10, max_epoch=150, max_prob=0.5):
            # gold_embs = text_model_output.decoder_hidden_states[-1][:,:-1,:]
            text_logits = text_model_output.logits[:,:-1,:]
            k = 5
            emb_weights, top_k_tgt = text_logits.topk(k, dim=-1)

            
            top_k_tgt = top_k_tgt.unsqueeze(-2)  # batch x seq_len x 1 x k
            ...
            weights = emb_weights.unsqueeze(3)  # batch x seq_len x k x 1
            
            k_embs = k_embs.squeeze(3).transpose(2, 3)  
            weights = emb_weights.unsqueeze(-1)  # [1024, 4, 5, 1]
        ...
        return normal_loss + tail_loss  +  0.5 * logit_loss + 0.1 * item_loss, normal_mean, tail_mean
    ...
    def evaluate(self, batch, index, train_mode='train', data_mode='eval'):
        ...
        encoder_output = id_model_output.encoder_last_hidden_state
        ...
        encoder_output_1 = text_model_output.encoder_last_hidden_state
        encoder_output_1 = encoder_output_1 * text_inputs['attention_mask'].unsqueeze(-1)
        encoder_output_1_mean = encoder_output_1.sum(dim=1) / text_inputs['attention_mask'].sum(dim=1).unsqueeze(-1)
        item_mean = id_model_output.decoder_hidden_states[-1][:,:-1,:].mean(dim=1)
        return id_model_output.loss 

    def predict(self, batch, prefix_allowed_tokens, test_mode='test', data_mode='test',model_type='tiger', index=None):
        self.eval()
        device = next(self.parameters()).device
        with torch.no_grad():
            self.text_emb = self.text_emb.to(device)
            ...
            if test_mode == 'test':
                ...
                np.save(f'../LETTER-TIGER-RAR/{model_type}/{self.dataset_name}/{data_mode}/{index}.mean.npy', encoder_output_mean.cpu().detach().numpy())
                np.save(f'../LETTER-TIGER-RAR/{model_type}/{self.dataset_name}/{data_mode}/{index}.decoder.mean.npy', decoder_output_mean.cpu().detach().numpy())
                np.save(f'../LETTER-TIGER-RAR/{model_type}/{self.dataset_name}/{data_mode}/{index}.text.mean.npy', text_model_output_mean.cpu().detach().numpy())
        return output

...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        if 'sim_labels' in batch[0].keys():
            sim_input = self.tokenizer([d['sim_input_ids'] for d in batch],
                                        return_tensors="pt",
                                        padding="longest",
                                        max_length=self.tokenizer.model_max_length,
                                        truncation=True,
                                        return_attention_mask=True)
            sim_labels = self.tokenizer([d['sim_labels'] for d in batch],
                                        return_tensors="pt",
                                        padding="longest",
                                        max_length=self.tokenizer.model_max_length,
                                        truncation=True,
                                        return_attention_mask=True)
            sim_input['labels'] = sim_labels['input_ids']
            sim_input['labels'][sim_input['labels'] == self.tokenizer.pad_token_id] = -100
            new_batch['sim_inputs'] = sim_input
...
class TestCollator(object):
    ...
    def __call__(self, batch):
        ...
        return (inputs, targets, text_input_ids, text_attention_mask)
